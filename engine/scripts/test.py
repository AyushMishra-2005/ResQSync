...
tokenizer = AutoTokenizer.from_pretrained(
    os.path.join(BASE_DIR, "../models/tokenizer")
)

# Load models
model_cat = BertForSequenceClassification.from_pretrained(
    os.path.join(MODEL_DIR, "category_model")
).to(device)

model_pri = BertForSequenceClassification.from_pretrained(
    os.path.join(MODEL_DIR, "priority_model")
).to(device)
...
# Load encoders
with open(os.path.join(MODEL_DIR, "cat_encoder.pkl"), "rb") as f:
    cat_encoder = pickle.load(f)

with open(os.path.join(MODEL_DIR, "pri_encoder.pkl"), "rb") as f:
